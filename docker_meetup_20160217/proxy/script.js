...
// Todo:
// Place replicationcontrollers and maybe change the color?
...
var connectControllers = function () {
        ...
        for (var j = 0; j < pods.items.length; j++) {
            ...
            if (matchesLabelQuery(pod.metadata.labels, controller.spec.selector)) {
                jsPlumb.connect({
                    ...
                    joinStyle: "round",
                    endpointStyle: {fillStyle: color, radius: 7},
    ...
    for (var i = 0; i < services.items.length; i++) {
        ...
        for (var j = 0; j < pods.items.length; j++) {
            ...
            var pod = pods.items[j];
            if ((pod.metadata.labels) && (pod.metadata.labels.name && service.spec.selector.name) && pod.metadata.labels.name == service.spec.selector.name) {
                jsPlumb.connect(
                    {
...
jsPlumb.bind("ready", function () {
    ...
    $.when(promise).then(refresh(instance));
